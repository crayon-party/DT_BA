...
public class BerthController : AllocatorAPIClient
{
    ...
    {
        // Destroy old ship GameObjects before creating new scenario
        if (portVisualizer != null)
            portVisualizer.ClearAllShips();
    ...
    // ── Weight sliders — wire each to OnAlphaChanged etc. ──────────────────
    // Or use OnWeightsChanged(a,b,g,d) from OperatorPanel directly

    public void OnWeightsChanged(float alpha, float beta, float gamma, float delta)
    {
        StartCoroutine(SetWeights(alpha, beta, gamma, delta, ok =>
        {
            if (ok) Debug.Log("[BerthController] Weights updated: " +
                              "α=" + alpha.ToString("F2") + " β=" + beta.ToString("F2") +
                              " γ=" + gamma.ToString("F2") + " δ=" + delta.ToString("F2"));
        }));
    }

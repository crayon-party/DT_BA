...
public class AllocatorAPIClient : MonoBehaviour
{
    ...
    /// <summary>
    /// Set multi-objective weights. Values are normalised server-side.
    /// alpha=shifting  beta=fatigue  gamma=delay  delta=emissions
    /// </summary>
    public IEnumerator SetWeights(float alpha, float beta, float gamma, float delta,
                                   Action<bool> onDone = null)
    {
        if (string.IsNullOrEmpty(SessionId)) { onDone?.Invoke(false); yield break; }
        string body = "{\"session_id\":\"" + SessionId + "\"," +
                      "\"alpha\":" + alpha.ToString("F4") + "," +
                      "\"beta\":" + beta.ToString("F4") + "," +
                      "\"gamma\":" + gamma.ToString("F4") + "," +
                      "\"delta\":" + delta.ToString("F4") + "}";
        var result = new HttpResult();
        yield return Post("/set_weights", body, result);
        onDone?.Invoke(result.Ok);
    }

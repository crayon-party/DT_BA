...
VESSEL_SPECS = {
    ...
    # Night penalty = fatigue x 10, remains significant but not prohibitive
    'K': {'readiness': 94, 'fatigue': 4.0, 'stay_range': (72, 96), 'tugs': 2, 'duration': 2, 'cycle': 504, 'count': 3,
          'assigned_piers': ['P1', 'P2', 'P7', 'P8'], 'weather_limit': 2, 'e_idle': 2134, 'e_underway': 10500},
...
class NavalFinalOptimizer:
    ...
    def evaluate_fitness(self, v, p_id, l_idx, t, wait_time):
        ...
        if self.mode == 'EDD':     return v.get('arr', 0) + v['stay'] * 2  # earliest due date
    ...
    def step(self, max_h=2000):
        ...
        # Sort waiting vessels by priority score when delta > 0
        # High delta → prioritise vessels with highest accumulated emissions
        # (largest vessel type × longest wait = most urgent to berth)
        delta = getattr(self, 'delta', 0.25)
        ...
        def vessel_priority(v):
            ...
            e_idle = VESSEL_SPECS[v['type']]['e_idle']
            return delta * e_idle * wt * 0.5  # higher = more urgent
        ...
        # Add non-waiting vessels (arr != t) — they won't be processed but need to be included

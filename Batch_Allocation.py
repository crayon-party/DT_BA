...
class NavalFinalOptimizer:
    def __init__(self, scenario, mode='GA', record_log=True):
        ...
        self.scenario = [v.copy() for v in scenario]
        self.mode = mode
    ...
    def calculate_vessel_fatigue(self, vessel_id, v_type, t, is_shifting=False):
        ...
        # [v11.1] 피로도 가중치를 상향하여 FCFS와의 변별력 확보
    ...
    def evaluate_fitness(self, v, p_id, l_idx, t, wait_time):
        ...
        # 지연 시간에 따른 보상 (대기 시간이 길어지면 입항 우선순위 상승)
        penalty -= (wait_time * 300)

        # Shifting 유발 시 강력한 페널티 (이동은 피로도와 지연 모두에 악영향)
        my_dep = t + (v['stay'] * 2)
        for i, other in enumerate(self.berths[p_id]):
            if other:
                ...
                if v['arr'] == t:
                    ...
                    if avail_tugs >= spec['tugs'] and w_lvl <= spec['weather_limit']:
                        ...
                        if best_p and best_s < 1e15:
                            ...
                            if self.record_log:
                                self.vessel_history.append({'Time': t / 2, 'VesselID': v['id'], 'Event': 'Arrival',
                                                            'Loc': f"{best_p}-{best_l}", 'Weather': w_lvl,
                                                            'Tugs': avail_tugs})
                        else:
                            v['arr'] += 1;
                            self.delay += 0.5
                        ...
                        v['arr'] += 1;
                        self.delay += 0.5
        return {'shifting': self.shifting, 'fatigue': self.fatigue, 'delay': self.delay, 'counts': dict(self.counts),
